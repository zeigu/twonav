...
$user = array(
		...
		'uid' => '88880929', //这里填写你的UID信息,用户授权UID，在 http://user.seakee.cn 平台可以查看

		'token' => '709f14f49e202d88c73dba3d8a6c2154', //这里填写你的用户密匙信息,用户授权TOKEN，在 http://user.seakee.cn 平台可以查看

		'path' => '/tool/ysjx', //一般不用修改,除非你放置在其他目录，修改格式 '/jiexi' (修改二级目录一定看好格式)，如果放在根目录就请留空，格式：'path' => '' 

		'domain' => 'wapxy.cn/tool/ysjx/?url=', //请设置您的解析域名（放解析客户端的域名），防止客户端被二开盗用数据（注意：此处不要添加 http:// 或 https://）
		
		'tiaodomain' => 'http://wapxy.cn', //请设置客户端被二开盗用后跳转的域名地址，被盗用后直接跳转到您设置的地址。
		
		'hdd' => '3', //视频默认清晰度，1标清，2高清，3超清，如果没有高清会自动下降一级（请保持默认，无需修改）
  
  		'p2p' => '0', //P2P加速,默认关闭,1:开启,0:关闭（为了不影响观看，开启也不会显示任何信息，不影响加速效果）
		...
		'ather' => '', //已为大家设置好备用接口,无特殊要求可以不用更改,如果不需要删除即可.//填写实例：'ather' => 'https://www.8090g.cn/?url=',

		'dplayer' => '', //用户设置dplayer播放器右键,不设置请留空。填写实例:'dplayer' => '8090解析,https://www.8090.la'
		
		'title' => '逍遥影视解析系统', //设置解析页面title名称   例如：'title' => '8090视频解析',
		...
		'lolink' => 'http://wapxy.cn', //用户设置防盗跳转,设置防盗验证时间lotime参数后才生效（设置顶部防盗链域名后生效，如果其他人盗用，会在指定时间跳转）  //填写实例:'lolink' => 'https://www.8090.la', 		

		'cklogo' => '', //ckplayer播放器右上角logo标志,请填写完整绝对地址.例如:https://www.8090.la/logo.png ,不用请留空
		...
		'hand' => '', //ckplayer播放器右键,例如:'hand'=>'8090解析,https://www.8090.la'
